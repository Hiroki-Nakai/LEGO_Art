...
def creat_instruction(color_map, depth_map, min_color_num, COLOR_DIC,image_id):
    ...
    if Width >= Height:
        sample_im_height = (Height*180/Width)
        cv.drawImage(image=image_path_full, x=15*mm, y=(200-sample_im_height/2)*mm, width=180*mm, height=sample_im_height*mm, mask='auto')
    else:
        sample_im_width = (Width*190/Height)
        cv.drawImage(image=image_path_full, x=(105-sample_im_width/2)*mm, y=100*mm, width=sample_im_width*mm, height=190*mm, mask='auto')
    #使用する色の総数を計算
    min_color_num_plusone = min_color_num+1
    depth_color_num = min_color_num_plusone*depth_map
    color_sum = [ int(sum(depth_color_num[min_color_num_plusone==i])//i)for i in range(1,len(COLOR_DIC)+1) ]
    Color_list = []
    for i, key in enumerate(COLOR_DIC):
        Color_list.append(COLOR_DIC[key])
    Color_len = len(Color_list)
    Color_len_syou = Color_len//table_len
    Color_len_syou_SUM = Color_len//table_len_color_SUM
    color_list_data = [[ j*table_len+i+1 for i in range(table_len)] for j in range(Color_len_syou)]
    color_SUM_data = [[ str(j*table_len_color_SUM+i+1)+"\n×\n"+str(color_sum[j*table_len_color_SUM+i]) for i in range(table_len_color_SUM)] for j in range(Color_len_syou_SUM)]
    ...
    if Color_len%table_len_color_SUM!=0:
        color_SUM_data += [[str(table_len_color_SUM*Color_len_syou_SUM+i+1)+"\n×\n"+str(color_sum[table_len_color_SUM*Color_len_syou_SUM+i]) for i in range(Color_len%table_len_color_SUM)]+[" "]*(table_len_color_SUM-Color_len%table_len_color_SUM)]
    table_color = Table(color_list_data, colWidths=(180//table_len)*mm, rowHeights=12*mm)
    table_color_SUM = Table(color_SUM_data, colWidths=(180//table_len_color_SUM)*mm, rowHeights=18*mm)
    ...
    table_color_SUM.setStyle(TableStyle([
    ...
    for y in range(len(color_list_data)):
        for x in range(table_len):
            if y*table_len+x < Color_len:
                ...
                break
    for y in range(len(color_SUM_data)):
        for x in range(table_len_color_SUM):
            if y*table_len_color_SUM+x < Color_len:
                b,g,r = Color_list[y*table_len_color_SUM+x]
                bgr = int(b)+int(g)+int(r)
                if bgr >= 382:
                    wordColor = colors.black
                else:
                ...
                break
    #使用する色の総数を表示
    table_color_SUM.wrapOn(cv, 15*mm, (5+(4-Color_len_syou_SUM)*18)*mm) # table位置
    table_color_SUM.drawOn(cv, 15*mm, (5+(4-Color_len_syou_SUM)*18)*mm)
    #改ページ
    cv.showPage()
